...
class GroupManagementController extends BaseController {
	...
	public function addMember($groupid){
		...
		$group_user_map = GroupUserMapping::where('groupid','=',$groupid)->where('userkey','=',$userkey)->get();
		if(!$group_user_map->isEmpty()){
			return Redirect::to('/group/'.$groupid)->with('error','ไม่สามารถเพิ่มสมาชิกได้ เนื่องจาก '.$screenname.' อยู่ในกลุ่มนี้แล้ว');
		}
